...
C = np.empty(int(w*h//size + (w*h % size > rank)), dtype='i')
N = w//size + (w % size > rank)

for j in range(h):
    for i in range(N*rank-N,N*(rank+1)-N*rank,1):
        x = xmin + i * (xmax-xmin)/w
        y = ymin + j * (ymax-ymin)/h
        C[j*h+i] = iterations_at_point(x, y, maxit)
print(rank)
print(C)
E = None
if rank == 0:
	E = np.empty(h*w,dtype='i')
comm.Gather(sendbuf=C,recvbuf=E)
if rank ==0:
	print(E)
	...
	for j in range(h):
    		for i in range(w):
        		pixels[i, j] = color(E[j*w+i], maxit)
if rank==0:
	image.save(filename)
